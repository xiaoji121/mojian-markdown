...
弹窗中点「阅读」进入独立阅读页：左栏是网页转出的 Markdown 源（可直接编辑，实时生效），右栏是渲染后的文章预览。色板与排版逐项对齐主编辑器的墨笺「墨」主题：仓耳今楷阅读衬线、2.1 倍行距、正文着色比标题柔和一层、琥珀色列表符号与行内代码、楷体斜排引文。构建时若本地已 `npm run font:fetch`，仓耳今楷子集（约 2MB）会自动打进插件；否则回退系统楷体/宋体。顶栏支持：
